...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    print ('23:{}', format(torch.cuda.memory_allocated(0)))
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            #loss_all['loss'].update(loss.detach().cpu().numpy())
            total_loss['loss'].update(loss.cpu().detach().numpy().item())
            print('c')
            optimizer.zero_grad()
            loss.backward()
            print('d')
            
            del  output_3D, out_target, loss, batch_cam, gt_3D, input_2D, action, subject, scale, bb_box, cam_ind, total_loss
    ...
    if split == 'train':
        return loss_all['loss'].avg.cpu().detach().numpy().item()#.detach().numpy.item()
    elif split == 'test':
    ...
    del output_3D, out_target, loss, batch_cam, gt_3D, input_2D, action, subject, scale, bb_box, cam_ind, total_loss
    torch.cuda.empty_cache()
    print ('24:{}', format(torch.cuda.memory_allocated(0)))
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opt.nepoch):
        if opt.train: 
            loss = train(opt, actions, train_dataloader, model, optimizer, epoch).cpu().detach().numpy().item()
